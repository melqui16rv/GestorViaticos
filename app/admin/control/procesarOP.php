...
function depurar_datos_op($archivo) {
    ...
    foreach ($lineas as $linea) {
        ...
        try {
            $datosDepurados[] = [
                'CODIGO_OP'                  => trim($datos[0]),
                'CODIGO_CRP'                 => trim($datos[1]),
                'CODIGO_CDP'                 => trim($datos[2]),
                'Numero_Documento'           => trim($datos[3]),
                'Fecha_de_Registro'          => parseDateOrExcelForDate($datos[4]), // Note el espacio en el nombre
                'Fecha_de_Pago'              => parseDateOrExcel($datos[5]),
                'Estado'                     => trim($datos[6]),
                'Valor_Bruto'                => $parseFloat($datos[7], $lineNumber, 'Valor_Bruto'),
                'Valor_Deducciones'          => $parseFloat($datos[8], $lineNumber, 'Valor_Deducciones'),
                'Valor_Neto'                 => $parseFloat($datos[9], $lineNumber, 'Valor_Neto'),
                'Tipo_Beneficiario'          => trim($datos[10]),
                'Vigencia_Presupuestal'      => trim($datos[11]),
                'Tipo_Identificacion'        => trim($datos[12]),
                'Identificacion'             => trim($datos[13]),
                'Nombre_Razon_Social'        => trim($datos[14]),
                'Medio_de_Pago'              => trim($datos[15]),
                'Tipo_Cuenta'                => trim($datos[16]),
                'Numero_Cuenta'              => trim($datos[17]),
                'Estado_Cuenta'              => trim($datos[18]),
                'Entidad_Nit'                => trim($datos[19]),
                'Entidad_Descripcion'        => trim($datos[20]),
                'Dependencia'                => trim($datos[21]),
                'Dependencia_Descripcion'    => trim($datos[22]),
                'Rubro'                      => trim($datos[23]),
                'Descripcion'                => trim($datos[24]),
                'Fuente'                     => trim($datos[25]),
                'Recurso'                    => trim($datos[26]),
                'Sit'                        => trim($datos[27]),
                'Valor_Pesos'                => $parseFloat($datos[28], $lineNumber, 'Valor_Pesos'),
                'Valor_Moneda'               => $parseFloat($datos[29], $lineNumber, 'Valor_Moneda'),
                'Valor_Reintegrado_Pesos'    => $parseFloat($datos[30], $lineNumber, 'Valor_Reintegrado_Pesos'),
                'Valor_Reintegrado_Moneda'   => $parseFloat($datos[31], $lineNumber, 'Valor_Reintegrado_Moneda'),
                'Tesoreria_Pagadora'         => trim($datos[32]),
                'Identificacion_Pagaduria'   => trim($datos[33]),
                'Cuenta_Pagaduria'           => trim($datos[34]),
                'Endosada'                   => trim($datos[35]),
                'Tipo_Identificacion2'       => trim($datos[36]),
                'Identificacion3'            => trim($datos[37]),
                'Razon_social'               => trim($datos[38]),
                'Numero_Cuenta4'             => trim($datos[39]),
                'Concepto_Pago'              => trim($datos[40]),
                'Solicitud_CDP'              => trim($datos[41]),
                'CDP'                        => trim($datos[42]),
                'Compromisos'                => trim($datos[43]),
                'Cuentas_por_Pagar'          => trim($datos[44]),
                'Fecha_Cuentas_por_Pagar'    => parseDateOrExcelForDate($datos[45]),
                'Obligaciones'               => trim($datos[46]),
                'Ordenes_de_Pago'            => trim($datos[47]),
                'Reintegros'                 => $parseFloat($datos[48], $lineNumber, 'Reintegros'),
                'Fecha_Doc_Soporte_Compromiso' => parseDateOrExcelForDate($datos[49]),
                'Tipo_Doc_Soporte_Compromiso' => trim($datos[50]),
                'Num_Doc_Soporte_Compromiso'  => trim($datos[51]),
                'Objeto_del_Compromiso'       => trim($datos[52])
            ];
        } catch (Exception $e) {
